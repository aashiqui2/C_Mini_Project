...
{
  Account acc;
  FILE *file = fopen(ACCOUNT_FILE, "ab+"); // appending binary red and write mode
  if (file == NULL)
  {
    printf("\nUnable to open file!!");
    return;
  ...
  //! This line take \n that is left over by the buffer
  char c;
  do
  {
    c = getchar();
...
void deposit_money()
{
  ...
  /*
    fread() reads 1 record of size sizeof(acc_r) from the file and stores it in acc_r.

   If fread() successfully reads a record, it returns 1 (true), so the while loop continues.

   If end-of-file (EOF) is reached or an error occurs, fread() returns 0, and the loop stops.
  */
  while (fread(&acc_r, sizeof(acc_r), 1, file))
  {
    if (acc_r.acc_no == acc_no)
    {
      ...
      //?-sizeof(acc_r) → Moves the pointer backward by the size of one acc_r record.

      //? SEEK_CUR → Moves the pointer relative to its current position.fseek(file, -sizeof(acc_r), SEEK_CUR);

      //! Move pointer back to overwrite the record
      fseek(file, -sizeof(acc_r), SEEK_CUR);

      //! fread() can be used to retrieve the data
