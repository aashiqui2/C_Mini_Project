...
#include <string.h>
#include <unistd.h>
#include <conio.h> // Windows alternative to termios.h
...
void input_credentials(char *username, char *password)
{
  ...
  int i = 0;
  char ch;
  // Available only in Windows (<conio.h>) 
  while ((ch = getch()) != '\r')//use getch not getchar()
  { // '\r' is Enter key
    if (ch == '\b' && i > 0)
    { // Handle backspace
      i--;
      printf("\b \b");// Move back, erase character, move back again
    }
    else if (ch != '\b')
    {
      password[i++] = ch;
